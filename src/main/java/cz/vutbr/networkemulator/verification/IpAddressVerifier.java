...
public class IpAddressVerifier extends InputVerifier {
        ...
        if (input instanceof JTextField field) {
            ...
            if (isValidIpv4Address || isValidIpv6Address) {
                ...
                JOptionPane.showMessageDialog(
                        ...
                        JMeterUtils.getLocaleString("Bad Address"),
    ...
    private static Pattern createIpv6Pattern() {
        ...
        ipv6Regex.append("fe80:(:[0-9a-fA-F]{0,4}){0,4}%[0-9a-zA-Z]{1,}|");     // fe80::7:8%eth0   fe80::7:8%1 (link-local IPv6 addresses with zone index)
        ipv6Regex.append("::(ffff(:0{1,4}){0,1}:){0,1}");
        ipv6Regex.append("((25[0-5]|(2[0-4]|1{0,1}[0-9]){0,1}[0-9])\\.){3,3}");
        ipv6Regex.append("(25[0-5]|(2[0-4]|1{0,1}[0-9]){0,1}[0-9])|");          // ::255.255.255.255   ::ffff:255.255.255.255  ::ffff:0:255.255.255.255  (IPv4-mapped IPv6 addresses and IPv4-translated addresses)
        ipv6Regex.append("([0-9a-fA-F]{1,4}:){1,4}:");
        ipv6Regex.append("((25[0-5]|(2[0-4]|1{0,1}[0-9]){0,1}[0-9])\\.){3,3}");
        ipv6Regex.append("(25[0-5]|(2[0-4]|1{0,1}[0-9]){0,1}[0-9]))");          // 2001:db8:3:4::192.0.2.33  64:ff9b::192.0.2.33 (IPv4-Embedded IPv6 Address)
